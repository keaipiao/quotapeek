...
面板会自动跟随 Codex 的界面语言，内置英文、简体中文和繁体中文；其他语言回退为英文。Codex Quota 会按低额度卡片的精确组件结构而不是翻译文字进行识别，并从启动开始在整个页面生命周期内隐藏普通侧栏、悬浮侧栏和窄窗口固定位置中的原生副本，不再依赖额度是否加载。我们的额度卡在同一个页面级节点中常驻并预先渲染；侧栏暂时不存在时只会隐藏停放，普通或悬浮侧栏出现前会把同一节点同步迁入，悬浮侧栏也会随原生展开动画一起显示。
