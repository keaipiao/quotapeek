...
function harness(overrides = {}) {
  ...
  const dependencies = {
    ...
    installNativeQuotaSuppression: async (options) => {
      events.push(["native-suppress", options]);
...
test("starts through the installed runtime and records verified daemon identity", async () => {
  ...
  const nativeSuppression = h.events.find(([name]) => name === "native-suppress");
  assert.deepEqual(nativeSuppression[1], {
    engineRoot: ENGINE,
    port: 55000,
    browserId: "browser-fixture",
    browserWebSocketUrl: "ws://127.0.0.1:55000/devtools/browser/browser-fixture"
  });
  assert.ok(h.events.findIndex(([name]) => name === "launch") < h.events.indexOf(nativeSuppression));
  assert.ok(h.events.indexOf(nativeSuppression) < h.events.indexOf(spawn));
});

test("native suppression is best-effort and cannot block daemon startup", async () => {
  const h = harness({
    dependencies: {
      installNativeQuotaSuppression: async () => {
...
test("a permanently pending native suppressor cannot delay daemon identity transfer", { timeout: 500 }, async () => {
  const h = harness({
    dependencies: {
      installNativeQuotaSuppression: () => new Promise(() => {}),
    },
  });
